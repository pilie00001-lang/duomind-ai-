...
import { Send, Plus, StopCircle, PlayCircle, MessageSquare, Menu, X, Sidebar, CheckCircle2, AlertTriangle, Code, Layout, BookOpen, Bell, Download, Archive, FolderDown } from 'lucide-react';
...
const getParticipantLabel = (sender: Sender): string => {
  switch (sender) {
    case Sender.Gemini: return 'Gemini 2.5 Flash';
    case Sender.Puter: return 'GPT-5.2 (Puter)';
    case Sender.Claude: return 'Claude 3.5 Haiku';
...
export default function App() {
  ...
  // Ref pour que les IA aient TOUJOURS le code le plus frais sans attendre le re-render React
  ...
  const getAIResponse = async (sender: Sender, history: Message[]): Promise<string> => {
    // Ordre Suprême : On injecte l'état EXACT de l'IDE à chaque appel
  ...
  useEffect(() => {
    ...
    const runLoop = async () => {
      ...
      try {
        ...
        setApiError("Boucle interrompue. Problème de connexion.");
        setIsAutoLoop(false);
      } finally {
  ...
  // NOUVEAU : Téléchargement réel de tous les fichiers avec leurs extensions
  const handleInstallProject = async () => {
    ...
    for (const name of fileNames) {
      ...
      link.href = url;
      link.download = name; // Garde l'extension .html, .py, .js...
      ...
      <div className={`fixed inset-y-0 left-0 z-30 w-64 bg-zinc-950 border-r border-zinc-800 transform transition-transform md:relative md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex flex-col h-full">
          <div className="p-4 border-b border-zinc-800 flex items-center justify-between">
            <h1 className="font-bold flex items-center gap-2 italic text-zinc-100">
              <Code className="text-blue-500" size={18} /> DuoMind
            </h1>
            <button onClick={() => setIsNewChatModalOpen(true)} className="p-2 hover:bg-zinc-800 rounded-full"><Plus size={20} /></button>
          </div>
          <div className="flex-1 overflow-y-auto p-2 space-y-1">
            {conversations.map(conv => (
              <div key={conv.id} onClick={() => setActiveId(conv.id)} className={`p-3 rounded-lg cursor-pointer truncate text-sm transition-all ${activeId === conv.id ? 'bg-zinc-900 border border-zinc-800' : 'text-zinc-500 hover:text-zinc-300'}`}>
                {conv.title}
                {conv.isCodeMode && <span className="ml-2 text-[8px] bg-blue-600 px-1 rounded font-black">IDE</span>}
              </div>
            ))}
          </div>
          <div className="p-4 border-t border-zinc-900 bg-zinc-950">
             <button 
              ...
              title="Télécharge chaque fichier (.py, .html...) individuellement"
              className="w-full flex items-center justify-center gap-2 py-3 bg-white hover:bg-zinc-200 text-black rounded-xl text-xs font-black transition-all shadow-[0_0_20px_rgba(255,255,255,0.1)]"
             >
        ...
        <header className="h-16 border-b border-zinc-800 flex items-center justify-between px-4 bg-zinc-950/50 backdrop-blur shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className="text-[10px] font-black text-blue-500 uppercase tracking-[0.2em]">{activeConversation.isCodeMode ? 'Mode IDE Activé' : 'Mode Chat'}</span>
              <div className="flex gap-4">
                 <button onClick={() => setViewMode('chat')} className={`text-xs font-black ${viewMode === 'chat' ? 'text-white underline underline-offset-4' : 'text-zinc-600'}`}>CONVERSATION</button>
                 <button onClick={() => setViewMode('code')} className={`text-xs font-black ${viewMode === 'code' ? 'text-white underline underline-offset-4' : 'text-zinc-600'}`}>CODE SOURCE</button>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button 
              ...
              className={`px-4 py-2 rounded-xl border text-[10px] font-black transition-all ${isAutoLoop ? 'bg-red-600 border-red-400 animate-pulse' : 'bg-zinc-900 border-zinc-800 text-zinc-400'}`}
            >
              {isAutoLoop ? 'COLLABORATION LIVE' : 'LANCER AUTO-RUN'}
            </button>
          </div>
        </header>
          ...
          {viewMode === 'chat' ? (
            <div className="h-full flex flex-col">
              <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6">
                ...
                {isProcessing && (
                  <div className="flex items-center gap-2 text-blue-500/50 italic text-[10px] font-bold">
              ...
              <div className="p-4 bg-zinc-950 border-t border-zinc-900">
                <form onSubmit={handleSend} className="max-w-4xl mx-auto flex gap-2">
                  <input 
                    ...
                    placeholder="Ex: 'Créez un script python qui génère un labyrinthe'..." 
                    className="flex-1 bg-zinc-900 px-4 py-3 rounded-xl border border-zinc-800 outline-none focus:border-blue-500 transition-all text-sm" 
                  />
                  <button type="submit" className="p-3 bg-white text-black rounded-xl hover:bg-blue-400 transition-colors">
                    <Send size={18} />
                  </button>
                </form>
              </div>
      ...
      {isNewChatModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/95 p-4 backdrop-blur-md">
          <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-3xl w-full max-w-md shadow-2xl">
            <h2 className="text-2xl font-black mb-6">Nouvelle Équipe AI</h2>
            <div className="space-y-3 mb-8">
              {[Sender.Gemini, Sender.Puter, Sender.Claude].map((sender) => (
                ...
                }} className={`w-full flex items-center justify-between p-5 rounded-2xl border transition-all ${selectedParticipants.includes(sender) ? 'bg-blue-600 border-blue-400' : 'bg-zinc-800 border-zinc-700 opacity-40'}`}>
                  <span className="font-bold">{getParticipantLabel(sender)}</span>
                  {selectedParticipants.includes(sender) && <CheckCircle2 size={20} />}
                </button>
              ))}
            </div>
            <div className="flex gap-4">
              <button onClick={() => setIsNewChatModalOpen(false)} className="flex-1 py-4 bg-zinc-800 rounded-2xl font-bold">ANNULER</button>
              <button onClick={() => {
                const newConv: Conversation = { id: uuidv4(), title: 'Nouveau Projet', messages: [], lastUpdated: Date.now(), participants: [...selectedParticipants], files: {}, isCodeMode: true };
                setConversations(prev => [newConv, ...prev]);
                setActiveId(newConv.id);
                setIsNewChatModalOpen(false);
              }} className="flex-1 py-4 bg-white text-black font-black rounded-2xl">LANCER</button>
            </div>
          </div>
        </div>
      )}
